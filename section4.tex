...
\[
    \Backstrom(L^c) = \argmax_{l^c_i \in L}
    \left(
        \prod_{l^c_j \in L^c} {\p(|l^c_i-l^c_j|) \over 1-\p(l^c_i-l^c_j) }
        \prod_{l^s_k \in L^s} 1-\p(|l^c_i-l^s_k|)
    \right)
\]
...
contacts:
\[
    \FriendLoc(L,P) = \argmax_{l^c_i \in L}
    \left(
        \pStrangers(l^c_i)
        \prod_{l^c_j \in L,p_j \in P}
        {\pContact(\quantile(p_j),|l^c_i-l^c_j|) \over (1-p(|l^c_i-l^c_j|))}
    \right)
\]
...
\begin{algorithm}
  ...
  \EndFor
  \State best = FriendLoc(locations,predictedDists)
  \State \Return locations[best]
  \end{algorithmic}
\end{algorithm}

\jam{locations of contacts are non-independent as seen in~\ref{sec:closer}. Where do we want to talk about this?}
