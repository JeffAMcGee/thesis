...
By comparing the number of edges that could have existed, $\edges(d)$, to the
number of edges that actually exist at a specific distance and in a specific
quantile, $\actEdges(j,d)$, we can find the probability that a contact at a
specific distance is in a quantile:
