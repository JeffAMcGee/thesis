...
\hyphenation{Friend-ly-Lo-ca-tion}
...
\item $D^a$ is the set of actual edge lengths from taget users to contacts
...
the decision tree.
%
Although the values returned by the regression tree are actually distances,
...
%using the millions of edges $\actEdges$ divided by billions of edges in $\stgrEdges$:
For each of the quantiles, we can fit this probality to the curve from
\cite{backstrom2010find}:
\[
...
\caption{
    ...
    \jam{consider removing this figure}
}
\label{fig:LocalCmp}
\vspace{-2pt}
\end{figure}
...
\caption{
    \jam{Reviewer asks: What do samples and value mean in this chart?}
